...
window.addPeserta = async function() {
  ...
  try {
    ...
    if (list) {
      ...
      div.style.cssText='padding:.3rem .5rem;border-radius:6px;margin-bottom:2px;background:var(--sf2)';
      div.innerHTML=`<span style="font-size:.82rem">${esc(nama)}</span><button class="btn btn-er btn-xs" onclick="window.dlPesertaInline('${id}',this)">✕</button>`;
